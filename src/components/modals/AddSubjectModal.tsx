...
import { useSubjectMutations } from '@/hooks/useSupabaseData';
...
export function AddSubjectModal({ onClose, editSubject }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editSubject) {
      updateSubject.mutate({ id: editSubject.id, ...form, schedule: editSubject.schedule });
    } else {
      addSubject.mutate({ ...form, schedule: [] });
    }
    onClose();
  };
      ...
      <div className="relative w-full max-w-lg card-surface shadow-2xl p-6 animate-scale-in max-h-[85vh] overflow-y-auto scrollbar-thin" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Carga Horária" value={String(form.credit_hours)} onChange={v => setForm({ ...form, credit_hours: Number(v) })} type="number" />
            <Field label="Créditos" value={String(form.credits)} onChange={v => setForm({ ...form, credits: Number(v) })} type="number" />
          </div>

          <div>
            <label className="block text-xs font-heading text-muted-foreground mb-1.5">Cor</label>
            <div className="flex gap-2">
              {SUBJECT_COLORS.map(c => (
                <button key={c} type="button" onClick={() => setForm({ ...form, color: c })} className={`h-7 w-7 rounded-full transition-all ${form.color === c ? 'ring-2 ring-primary ring-offset-2 ring-offset-card' : ''}`} style={{ backgroundColor: c }} />
              ))}
            </div>
          </div>
          ...
          <Field label="Link do AVA" value={form.ava_link} onChange={v => setForm({ ...form, ava_link: v })} placeholder="https://" />
...
function Field({ label, value, onChange, placeholder, type = 'text' }: {
  ...
  return (
    <div>
      <label className="block text-xs font-heading text-muted-foreground mb-1.5">{label}</label>
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="w-full h-9 px-3 rounded-md bg-secondary border border-border text-sm text-foreground placeholder:text-muted-foreground/60 glow-focus" />
    </div>
  );
}
