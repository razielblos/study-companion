...
export default function Progresso() {
  ...
  const subjectData = useMemo(() => activeSubjects.map((subj: any) => {
    const evals = evaluations.filter((e: any) => e.subject_id === subj.id && e.score !== null);
    const tw = evals.reduce((s: number, e: any) => s + e.weight, 0);
    const ws = evals.reduce((s: number, e: any) => s + (e.score || 0) * e.weight, 0);
    const avg = tw > 0 ? ws / tw : null;
    const subjTasks = tasks.filter((t: any) => t.subject_id === subj.id);
    const done = subjTasks.filter((t: any) => t.status === 'concluída').length;
  ...
  const taskBarData = subjectData.map(s => ({ name: s.name, concluídas: s.done, pendentes: s.total - s.done }));
    ...
    <div className="max-w-5xl space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card-surface p-4">
          <p className="text-xs font-heading text-muted-foreground mb-1">Disciplinas Ativas</p>
          <p className="text-2xl font-heading font-bold text-foreground">{activeSubjects.length}</p>
        </div>
        <div className="card-surface p-4">
          <p className="text-xs font-heading text-muted-foreground mb-1">Média Geral</p>
          <p className={`text-2xl font-heading font-bold ${overallAvg !== null ? (overallAvg >= 7 ? 'text-success' : overallAvg >= 5 ? 'text-warning' : 'text-destructive') : 'text-muted-foreground/60'}`}>
            {overallAvg !== null ? overallAvg.toFixed(1) : '—'}
          </p>
        </div>
        <div className="card-surface p-4">
          <p className="text-xs font-heading text-muted-foreground mb-1">Tarefas Concluídas</p>
          <p className="text-2xl font-heading font-bold text-foreground">
            {tasks.filter((t: any) => t.status === 'concluída').length}/{tasks.length}
          </p>
        </div>
        <div className="card-surface p-4">
          <p className="text-xs font-heading text-muted-foreground mb-1">Créditos do Semestre</p>
          <p className="text-2xl font-heading font-bold text-primary font-mono">
            {activeSubjects.reduce((s: number, subj: any) => s + subj.credits, 0)}
          </p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              <th className="text-left px-5 py-3 font-heading text-muted-foreground text-xs">Disciplina</th>
              <th className="text-center px-3 py-3 font-heading text-muted-foreground text-xs">Créditos</th>
              <th className="text-center px-3 py-3 font-heading text-muted-foreground text-xs">Média</th>
              <th className="text-center px-3 py-3 font-heading text-muted-foreground text-xs">Tarefas</th>
              <th className="text-right px-5 py-3 font-heading text-muted-foreground text-xs">Status</th>
            </tr>
          </thead>
          <tbody>
            {subjectData.map(s => (
              <tr key={s.fullName} className="border-b border-border/50">
                ...
                <td className="text-center px-3 py-3 text-muted-foreground font-mono">{s.credits}</td>
                <td className={`text-center px-3 py-3 font-mono font-semibold ${s.avg !== null ? (s.avg >= 6 ? 'text-success' : 'text-destructive') : 'text-muted-foreground/60'}`}>
                  {s.avg !== null ? s.avg.toFixed(1) : '—'}
                </td>
                <td className="text-center px-3 py-3 text-muted-foreground font-mono">{s.done}/{s.total}</td>
                <td className="text-right px-5 py-3">
                  <span className={`text-xs font-heading ${s.avg !== null ? (s.avg >= 6 ? 'text-success' : 'text-destructive') : 'text-muted-foreground'}`}>
